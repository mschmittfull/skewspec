...
from lsstools.sim_galaxy_catalog_creator import SimGalaxyCatalogCreator
from nbodykit.lab import *
from read_utils import read_delta_from_rho_bigfile, read_delta_from_1plusdelta_bigfile, read_vel_from_bigfile, readout_mesh_at_cat_pos, read_delta_from_2plusdelta_bigfile


# TODO: Should move this to lsstools.
...
class Target(object):
    ...
    def get_catalog(self, keep_all_columns=False):
        ...
        if self.in_fname is None:
            ...
            if comm.rank == 0:
                print('Read %s' % self.in_fname)
            if not os.path.exists(self.in_fname):
                raise Exception('Could not find file %s' % self.in_fname)
